    ...
    , m_reconnect(new QTimer(this))
{
    ...
    connect(m_socket, &QLocalSocket::errorOccurred, this, [this](QLocalSocket::LocalSocketError) { scheduleReconnect(); });
...
void NulVoiceChatListener::onReadyRead()
{
    ...
    while ((newline = m_buffer.indexOf('\n')) >= 0) {
        ...
        Q_EMIT transcriptReceived(text, obj.value(QStringLiteral("app")).toString(), obj.value(QStringLiteral("mode")).toString());
    }
}
